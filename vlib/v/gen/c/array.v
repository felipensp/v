...
fn (mut g Gen) fixed_array_init(node ast.ArrayInit, array_type Type, var_name string, is_amp bool) {
	...
	if node.has_index {
		...
		index_typ_str := g.styp(node.index_type)
		if var_name == '' {
		...
		g.writeln('for (${index_typ_str} index=0; index<_len; index++, pelem++) {')
		g.set_current_pos_as_last_stmt_pos()
		g.indent++
		g.writeln('${index_typ_str} it = index;') // FIXME: Remove this line when it is fully forbidden
		g.write('*pelem = ')
...
fn (mut g Gen) array_init_with_fields(node ast.ArrayInit, elem_type Type, is_amp bool, shared_styp string,
	...
	if node.has_index {
		...
		g.writeln('for (${index_typ_str} index=0; index<${past.tmp_var}.len; index++, pelem++) {')
		g.set_current_pos_as_last_stmt_pos()
		g.indent++
		g.writeln('${index_typ_str} it = index;') // FIXME: Remove this line when it is fully forbidden
		g.write('*pelem = ')
